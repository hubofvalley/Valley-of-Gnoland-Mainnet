...
for command_name in curl git jq sha256sum gzip; do
...
[ "$CURRENT_GNOLAND_SHA" = "$GNOLAND_ASSET_SHA256" ] && GNOLAND_NEEDS_UPDATE=false
[ "$CURRENT_GNOKEY_SHA" = "$GNOKEY_ASSET_SHA256" ] && GNOKEY_NEEDS_UPDATE=false
...
if ! $GNOLAND_NEEDS_UPDATE && ! $GNOKEY_NEEDS_UPDATE && ! $SOURCE_NEEDS_UPDATE && ! $GENESIS_WAS_MISSING; then
    echo -e "${GREEN}Already up to date.${RESET}"
    echo "gnoland: $ASSET_VERSION ($GNOLAND_ASSET_SHA256)"
    echo "gnokey:  $ASSET_VERSION ($GNOKEY_ASSET_SHA256)"
...
CURRENT_STAGE="stage and verify release assets"
...
echo "  gnoland: ${CURRENT_GNOLAND_SHA:-missing} -> $GNOLAND_ASSET_SHA256"
echo "  gnokey:  ${CURRENT_GNOKEY_SHA:-missing} -> $GNOKEY_ASSET_SHA256"
...
mkdir -p "$(dirname "$GNOLAND_BIN")" "$(dirname "$GNOKEY_BIN")" "$GNOLAND_DEPLOYMENT_DIR"
...
verify_reported_version "$GNOLAND_BIN" "$GNOLAND_ASSET" gnoland
verify_reported_version "$GNOKEY_BIN" "$GNOKEY_ASSET" gnokey
